...
public class GeneratePositions implements IGeneratePositions {
  ...
  private byte[] HexStringToByteArray(String s) {
    ...
    for (int i = 0; i + 1 < len; i += 2) {
      data[i / 2] =
          (byte) ((Character.digit(s.charAt(i), Constants.OFFSET_16) << Constants.OFFSET_4)
              + Character.digit(s.charAt(i + Constants.OFFSET_1), Constants.OFFSET_16));
    }
    return data;
  }
  ...
  private TreeSet<Coordinate<?, ?>> GetAllPositions(String hashStr, int significantBits,
      int maxPositions) {
    TreeSet<Coordinate<?, ?>> positions = new TreeSet<>();
    String hashByteMissing = hashStr;
    do {
      positions
          .addAll(GetPositions(hashByteMissing, significantBits, maxPositions - positions.size()));
      hashByteMissing = EncryptSHA512(hashByteMissing);
    } while (positions.size() < maxPositions);
    return positions;
  }

  private TreeSet<Coordinate<?, ?>> GetPositions(String hashStr, int significantBits,
      int maxPositions) {
    ...
    for (int i = 0; i < hashByteLength && positions.size() < maxPositions; i++) {
      ...
      if (x != 0 && y != 0) {
        Coordinate<Integer, Integer> coordinate = new Coordinate<>(x, y);
